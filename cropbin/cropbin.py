...
"""
    ...
    Caution:
        - Not sure if multiplane or multichannel recordings are supported.
...
"""
    Parameters
"""
INPUT_DIR = "E:/GFAP;GCamp6s"
OUTPUT_DIR = os.path.join("..", "..", "processed")

CFG_FNAME = "GFAP_GCamp6s.ini"
...
""" EXP_NAMES = [
    "20210917_10_44_27_dR_ctrl1",
    "20210917_11_59_45_dR_PTZ1_2",
] """
CROP_IDS = ["Forebrain"]
...
def read_config(fname, exp_names, crop_ids):
    ...
    for sec in cfg.sections():
        ...
        crop_x_start, crop_x_stop = crop_x_mid - int(crop_lx / 2), int(crop_x_mid + crop_lx / 2)
        crop_y_start, crop_y_stop = int(crop_y_mid - crop_ly / 2), int(crop_y_mid + crop_ly / 2)
...
def rw_loop(raw_path_in, raw_path_out, rawr: RawReader):
    with open(raw_path_out, "wb") as raw_out:
        with open(raw_path_in, "rb") as raw_in:
            rawr.read_skip_frames(raw_in)
            ...
            for _ in range(rawr.n_batches):
                for i in range(rawr.tb_per_batch):
                    ...
                    tmp1 = np.reshape(
                        in_data,
                        (rawr.cfg_in.Ly, rawr.cfg_in.Lx, rawr.tb_size, rawr.n_planes),
                    )
                    tmp2[:, :, i] = np.mean(
                        tmp1[
                ...
                out_data = np.reshape(tmp2, (-1, rawr.tb_per_batch * rawr.n_planes))
                raw_out.write(out_data.tobytes(order="F"))

            # Lots of duplicate code here
            for i in range(rawr.n_rest // (rawr.tb_size * rawr.n_planes)):
                in_data = rawr.read_time_bin(raw_in, in_data)
            ...
            out_data = np.reshape(
                tmp2[:, :, : rawr.n_rest // rawr.tb_size],
                (-1, (rawr.n_rest // rawr.tb_size) * rawr.n_planes),
            )
            raw_out.write(out_data.tobytes(order="F"))
...
def cropbin(rawr: RawReader, start_frame=None, end_frame=None):
    ...
    rawr.calc_cfg_out_and_derived(start_frame, end_frame)
...
def main():
    ...
    count = 0
    tmp_list = []
    for i, rawr in enumerate(rawr_list):
        tmp_list.append(rawr)
        count += 1

        if count == 5 or i == len(rawr_list) - 1:
            rawr_list_list.append(tmp_list)
            count = 0
            tmp_list = []
        

    for rawr_list in rawr_list_list: """
    threads = []

    for rawr in rawr_list:
        print(rawr.exp_name)
        t = threading.Thread(target=cropbin, args=(rawr, START_FRAME, END_FRAME))
        threads.append(t)
        t.start()
