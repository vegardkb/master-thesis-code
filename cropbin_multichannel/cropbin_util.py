...
class RawReader:
    ...
    def calc_cfg_out_and_derived(self):
        ...
        const_bits = (
            2 * self.bits_per_pix * self.cfg_out.n_pix * self.tb_size * self.n_planes * self.n_ch
        )
        bits_remaining = max_memory_alloc_bits - const_bits
        self.tb_per_batch = bits_remaining // (
            2 * self.bits_per_pix * self.cfg_out.n_pix * self.n_planes * self.n_ch
        )
        self.batch_size = self.tb_per_batch * self.tb_size * self.n_planes * self.n_ch
        self.starting_frame = self.cfg_in.n_frames % (self.tb_size * self.n_planes * self.n_ch)
        n_frames_read = self.cfg_in.n_frames - self.starting_frame
    ...
    def init_arrays(self):
        in_data = np.zeros(
            (self.cfg_in.n_pix, self.tb_size, self.n_planes, self.n_ch), dtype=self.cfg_in.dtype
        )
        tmp1 = np.zeros(
            (self.cfg_in.Ly, self.cfg_in.Lx, self.tb_size, self.n_planes, self.n_ch),
            dtype=self.cfg_in.dtype,
        )
        tmp2 = np.zeros(
            (self.cfg_out.Ly, self.cfg_out.Lx, self.tb_per_batch, self.n_planes, self.n_ch),
            dtype=self.cfg_out.dtype,
        )
        out_data = np.zeros(
            (self.cfg_out.n_pix, self.tb_per_batch * self.n_planes * self.n_ch),
            dtype=self.cfg_out.dtype,
        )
